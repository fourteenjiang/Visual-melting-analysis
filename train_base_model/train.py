...
import C3D_model, R2Plus1D_model, R3D_model
...
lr = 1e-2 # try Learning rate 0.005?tried  5e-3 1e-3 5e-4
num_classes=3
patience=18
delta=5e-3
batch_size=16
...
def train_model(model, optimizer, scheduler,trainval_loaders, num_epochs=nEpochs, patience=patience, delta=delta):
        """
        Args:
...
def initialize_model(modelName,  num_classes, lr, pretrained, feature_extractor):
    """
       Args:
    ...
    if modelName == 'C3D':
        ...
        model = R2Plus1D_model.R2Plus1DClassifier(num_classes=num_classes, layer_sizes=(2, 2, 2, 2), pretrained=pretrained)
        ...
        model = R3D_model.R3DClassifier(num_classes=num_classes, layer_sizes=(2, 2, 2, 2), pretrained=pretrained)
...
if __name__ == "__main__":
  for modelName in ['C3D']:
      for feature_extractor in [False]:
        pretrained=True
        acc_ls = []
        f1score=[]
        save_dir = os.path.join(os.getcwd(), modelName + '_result ' + type_video, ' FE' if feature_extractor else 'FT')
        check_and_create_directory(save_dir)
        Log_result = (os.path.join(save_dir, 'log.txt'))
        fnames, labels = get_label_and_video_name(Path.db_dir(type_video)[2])[1], get_label_and_video_name(Path.db_dir(type_video)[2])[0]
        ...
        for train_index, val_index in skf.split(X=fnames, y=labels):
            ...
            train_dataloader = DataLoader(
                VideoDataset(split='train', fnames=fnames_train, labels=labels_train, type_video=type_video, clip_len=16,
            ...
            test_dataloader = DataLoader(
                VideoDataset(split='test', fnames=fnames_test_meta, labels=labels_test_meta, type_video=type_video, clip_len=16,
                             process=True),
            ...
            model, parameters =initialize_model(modelName, num_classes=num_classes, lr=lr, pretrained=pretrained, feature_extractor=feature_extractor)
            ...
            model.to(device)
            criterion.to(device)
            model_ft, hist, h, best_acc = train_model(model=model, optimizer=optimizer, scheduler=scheduler,trainval_loaders=trainval_loaders)


            plot_history(h, trial)
            test_model(model=model_ft, test_dataloader=test_dataloader)
