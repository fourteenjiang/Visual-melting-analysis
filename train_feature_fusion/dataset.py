...
class VideoDataset(Dataset):
    ...
    def __init__(self, split, fnames, labels, clip_len=16, process=True):
        #  root_dir =[ 'IR_video','RGB_video']
        self.root_dir, self.output_dir, self.label_path = Path.db_dir()
        ...
        if process:
           for i in range(2):
                ...
                if split == 'train':
                        os.mkdir(os.path.join(self.output_dir[i], 'train'))
                elif split == 'val':
                        os.mkdir(os.path.join(self.output_dir[i], 'val'))
                else:
                        os.mkdir(os.path.join(self.output_dir[i], 'test'))
                for video in self.fnames:
                    self.process_video(video, self.root_dir[i], os.path.join(self.output_dir[i], split))
    ...
    def __getitem__(self, index):
        ...
        buffer_ir , buffer_rgb = self.load_frames(os.path.join( self.output_dir[0], self.split, self.fnames[index])),self.load_frames(os.path.join( self.output_dir[1], self.split, self.fnames[index]))
        buffer_ir, buffer_rgb = self.crop(buffer_ir, self.clip_len, self.crop_size),  self.crop(buffer_rgb , self.clip_len, self.crop_size)
        labels = np.array(self.labels[index])

        if self.split == 'train':
            # Perform data augmentation
            buffer_ir = self.randomflip(buffer_ir)
            buffer_rgb = self.randomflip(buffer_rgb)
        buffer_ir, buffer_rgb = self.normalize(buffer_ir), self.normalize(buffer_rgb)
        buffer_ir, buffer_rgb = self.to_tensor(buffer_ir), self.to_tensor(buffer_rgb)
        # print(buffer.shape)
        return torch.from_numpy(buffer_ir), torch.from_numpy(buffer_rgb),torch.from_numpy(labels)

    def check_integrity(self):
        if not os.path.exists(self.root_dir[0]):
            return False
        elif not os.path.exists(self.root_dir[1]):
            return False
        else:
            return True
    ...
    def process_video(self, video, root_dir, save_dir):
        ...
        capture = cv2.VideoCapture(os.path.join(root_dir, video))
...
if __name__ == "__main__":
    ...
    fnames,labels= get_label_and_video_name(Path.db_dir()[2])[1],get_label_and_video_name(Path.db_dir()[2])[0]
    ...
    for train_index, val_index in skf.split(X=fnames, y=labels):
        ...
        train_data = VideoDataset(split='train', fnames=fnames_train, labels=labels_train, clip_len=16, process=True)
        ...
        shutil.rmtree(os.path.join(Path.db_dir()[1],'train'))
