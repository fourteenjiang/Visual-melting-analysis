...
class C3D(nn.Module):
    ...
    def __load_pretrained_weights(self):
        ...
        p_dict = torch.load(Path.c3dmodel_dir())
...
class C3DFeatureExtractor(nn.Module):
    def __init__(self, num_classes, pretrained=False):
        ...
        # 初始化R3DNet模型
        self.c3dnet = C3D(num_classes, pretrained=pretrained)
        self.c3dnet.to("cuda")

    def forward(self, x):
        # 使用R3DNet模型进行前向传播，获取分类logits和特征
        logits, early_fusion_feature, late_fusion_feature = self.c3dnet(x)
        # 根据需求返回早期和晚期特征
...
if __name__ == "__main__":
    ...
    net = C3D(num_classes=3, pretrained=True)



    feature_extractor = nn.Sequential(*list(net.children())[:-3])
    ...
    for param in classifier.parameters():
      print(param)
    # logits, features = c3d_model(input_data)
    # mid_features = features['mid_features']
